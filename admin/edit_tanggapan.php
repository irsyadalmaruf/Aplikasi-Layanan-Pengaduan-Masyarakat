...
<head>
  ...
  <title>APPEM - Ubah Tanggapan</title>
...
<a href="?url=lihat_tanggapan" class="kiri btn btn-info btn-icon-split"><span class="icon text-white-60">
  <i class="kiri fas fa-arrow-left"></i></span></a>
<h5 class="tengah m-1 font-weight-bold text-info">Ubah Tanggapan</h5>
</div>
<div class="card-body">
  ...
  <form action="update_tanggapan.php" method="post" class="form-horizontal" enctype="multipart/form-data">
      ...
      <input type="text" name="id_pengaduan" value="<?php echo $data['id_pengaduan']; ?>" class="form-control" readonly></div>
    ...
    <div class="form-group cols-sm-6">
     ...
     <div class="form-group cols-sm-6">
      ...
      <textarea class="form-control" rows="7" name="isi_laporan"><?php echo $data['tanggapan']; ?></textarea>
    </div>
    </div>
    ...
    <center>
      <div class="card-header py-3">
      <input type="submit" value="Simpan Perubahan" class="btn btn-info btn-user" onclick="return confirm('Anda Yakin Ingin Mengubah Tanggapan?')">
    </div></center>
  </div>
</form>
<?php } ?>
